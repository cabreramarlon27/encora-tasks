...
//         Add default tasks if needed
        if (taskRepository.count() == 0) {
            List<Task> defaultTasks = Arrays.asList(
                    new Task("1", "Learn Spring Boot", "Study Spring Boot fundamentals", LocalDate.now(), TaskStatus.TODO, "admin2"),
                    new Task("2", "Build a REST API", "Create a REST API with Spring Boot", LocalDate.now(), TaskStatus.IN_PROGRESS, "admin2")
            );
